...
export class DashboardRenderer {
    ...
    initSpotlight() {
        ...
        document.addEventListener('pointermove', (e) => {
            const card = e.target.closest('.categoria, .mini-widget-card, .search-box-wrapper, .settings-drawer-card, .hud-modal-card');
            if (!card) return;
            const rect = card.getBoundingClientRect();
            ...
            if (card.classList.contains('mini-widget-card') || card.classList.contains('search-box-wrapper') || card.classList.contains('settings-drawer-card') || card.classList.contains('hud-modal-card')) {
